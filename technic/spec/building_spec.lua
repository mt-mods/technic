...
describe("Power network building", function()
	...
	describe("cable building", function()
		...
		it("does not crash", function()
			...
			assert.equals(3, mineunit.utils.count(net.all_nodes))
		...
		it("adds all network nodes", function()
			assert.equals(6, mineunit.utils.count(net.all_nodes))
		end)
	...
	describe("cable building to machine", function()
		...
		it("does not crash", function()
			assert.equals(1, #net.PR_nodes)
			assert.equals(4, mineunit.utils.count(net.all_nodes))
			world.set_node(build_pos, {name="technic:hv_cable", param2=0})
			technic.place_network_node(build_pos, {"HV"}, "technic:hv_cable")
		end)
		...
		it("does not add unconnected cables to network", function()
			assert.is_nil(net.all_nodes[core.hash_node_position({x=103,y=812,z=100})])
		end)
		...
		it("does not duplicate already added machine", function()
			...
			assert.equals(2, #net.PR_nodes)
			assert.equals(7, mineunit.utils.count(net.all_nodes))
		end)
	...
	describe("machine building", function()
		...
		it("does not remove connected machines from network", function()
			...
			assert.same({x=103,y=821,z=100},net.all_nodes[core.hash_node_position({x=103,y=821,z=100})])
		end)
	...
	describe("cable cutting", function()
		...
		it("removes connected cables from network", function()
			--assert.is_nil(net.all_nodes[core.hash_node_position({x=103,y=900,z=100})])
			assert.equals(3, count(net.all_nodes))
		end)

		it("removes connected machines from network", function()
			--assert.is_nil(net.all_nodes[core.hash_node_position({x=103,y=901,z=100})])
			assert.equals(1, #net.PR_nodes)
		end)
		--]]
	...
	describe("cable digging below machine", function()
		...
		it("removes connected machines from network", function()
			assert.is_nil(net.all_nodes[core.hash_node_position({x=103,y=911,z=100})])
			assert.equals(1, #net.PR_nodes)
		end)
		...
		it("keeps connected machines in network", function()
			...
			assert.same({x=101,y=911,z=100}, net.all_nodes[core.hash_node_position({x=101,y=911,z=100})])
			assert.equals(1, #net.PR_nodes)
	...
	describe("machine digging", function()
		...
		it("does not remove connected machines from network", function()
			assert.same({x=101,y=921,z=100},net.all_nodes[core.hash_node_position({x=101,y=921,z=100})])
			assert.same({x=103,y=921,z=100},net.all_nodes[core.hash_node_position({x=103,y=921,z=100})])
			assert.equals(2, #net.PR_nodes)
		end)
