	...
	core.register_tool(data.can_name, {
		...
		on_use = function(itemstack, user, pointed_thing)
			...
			if core.is_protected(pointed_thing.under, user:get_player_name()) then
				core.log("action", user:get_player_name().." tried to take "..node.name..
					" at protected position "..core.pos_to_string(pointed_thing.under).." with a "..data.can_name)
				return
			end
		...
		on_place = function(itemstack, user, pointed_thing)
			...
			if def.on_rightclick and user and not user:get_player_control().sneak then
				return def.on_rightclick(pos, core.get_node(pos), user, itemstack, pointed_thing)
			end
			if not def.buildable_to or node_name == data.liquid_source_name then
				pos = pointed_thing.above
				node_name = core.get_node(pos).name
				def = core.registered_nodes[node_name] or {}
			...
			if core.is_protected(pos, user:get_player_name()) then
				core.log("action", user:get_player_name().." tried to place "..data.liquid_source_name..
					" at protected position "..core.pos_to_string(pos).." with a "..data.can_name)
				return
			end
